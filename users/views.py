...
from .forms import NewUserForm, AddAttendanceMember
...
def attendance(request):
	...
	username_lists = Users.objects.values_list('name',flat =True)
	existed_Data = Attendances.objects.count()
	data_To_Transfer =Users.objects.count()

	if existed_Data < data_To_Transfer:
		for name_value in username_lists:
			add_name = Attendances(User_id = name_value)
			add_name.save()


	attendance_list = Attendances.objects.order_by('id')
	context = {
...
def add_attendance_member(request):
	context = {}

	if request.method == 'POST':
		form = AddAttendanceMember(request.POST)

		if form.is_valid():
			newMember = Attendances(User_id=form.cleaned_data['name'],
									Status=form.cleaned_data['status'],
									)
			newMember.save();
			context['success_message'] = 'New Member added'

	context['form'] = AddAttendanceMember()

	return render(request, 'users/add_attendance_member.html', context)
